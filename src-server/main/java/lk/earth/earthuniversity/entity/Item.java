...
public class Item {
    ...
    @Basic
    @Column(name = "name")
    @Pattern(regexp = "^[A-Z][a-z]+\\s[A-Z][a-z]+\\s(?:[A-Z][a-z]+\\s)?[\\d]{1,3}(kg|g|l|ml)$",message ="Invalid Name")
    private String name;
    @Basic
    @Column(name = "code")
    ...
    @Basic
    @Column(name = "sprice")
    @Pattern(regexp = "^\\d{2,4}(?:[.]\\d{2})?$",message ="Invalid Sale Price")
    private BigDecimal sprice;

    @Basic
    @Column(name = "pprice")
    @Pattern(regexp = "^\\d{2,4}(?:[.]\\d{2})?$",message ="Invalid Purches Price")
